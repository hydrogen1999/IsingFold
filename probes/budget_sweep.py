"""Does a larger embedding of the same problem solve it better?

Every earlier attempt at this compared the candidates a state offers, which sit about five qubits
apart and are local perturbations of one embedding. That is not a budget sweep, and the claim
built on it, that the resource-quality premise fails, was withdrawn.

This builds the comparison properly. One logical instance, one minorminer embedding, then several
grown versions of it whose chain lengths are drawn from power laws with different exponents: a
steep exponent keeps chains short and the embedding small, a shallow one draws long chains and
spends qubits. Every version is a valid embedding of the same problem with the same couplings, so
what changes between them is resource and nothing else.

Then each is measured on its own independent block, and the question is whether quality rises,
falls or does nothing as the qubit count rises. Because the growth only adds qubits, the versions
are nested in resource, which is the condition under which the advisor's claim, that unused
hardware is an opportunity rather than waste, is a claim about something measurable.
"""
import argparse, json, os, sys, time
from pathlib import Path

sys.path.insert(0, os.environ["ISINGFOLD_SRC"])
sys.path.insert(0, str(Path(__file__).resolve().parent))
sys.meta_path[:] = [f for f in sys.meta_path
                    if not ("editable" in (getattr(type(f), "__module__", "") or "").lower()
                            and "isingfold" in (getattr(type(f), "__module__", "") or "").lower())]
import numpy as np
from isingfold.rl.contracts import Context
from isingfold.rl.data.generate import load_instances
from isingfold.rl.env import fixed_strength_selector
from isingfold.rl.evaluate import first_commit_controller, run_controller

from _initializers import minorminer_initializer
from powerlaw_embeddings import grown_embedding, realises

MEASURE_BASE = 20_000_000


def main() -> int:
    ap = argparse.ArgumentParser()
    ap.add_argument("--corpus", required=True)
    ap.add_argument("--lineages", type=int, default=80)
    ap.add_argument("--alphas", default="4.0,3.0,2.2,1.6,1.2",
                    help="steep to shallow; a steep exponent keeps chains short")
    ap.add_argument("--modes", default="length,redundant,contact",
                    help="how the extra qubits are spent: lengthening a chain, or closing cycles "
                         "inside it. At equal cost these are different spends, and only the "
                         "second is what spending strategically could mean")
    ap.add_argument("--lmin", type=int, default=1)
    ap.add_argument("--lmax", type=int, default=8)
    ap.add_argument("--reads", type=int, default=512)
    ap.add_argument("--repeats", type=int, default=2,
                    help="growth draws per exponent, so one unlucky draw is not the arm")
    ap.add_argument("--qubit-cap", type=int, default=120)
    ap.add_argument("--seed", type=int, default=0)
    a = ap.parse_args()

    tasks = load_instances(a.corpus)[: a.lineages]
    ctx = Context(qubit_cap=a.qubit_cap)
    mm = minorminer_initializer(20)
    alphas = [float(x) for x in a.alphas.split(",")]
    modes = [m.strip() for m in a.modes.split(",") if m.strip()]
    rng = np.random.default_rng(a.seed)
    print(json.dumps({"corpus": a.corpus, "lineages": len(tasks), "alphas": alphas,
                      "modes": modes, "lmin": a.lmin, "lmax": a.lmax, "reads": a.reads,
                      "repeats": a.repeats}), flush=True)

    def measure(task, chains, tag):
        def fixed(l, h, s):
            return chains
        try:
            out = run_controller([task], ctx, first_commit_controller, initializer=fixed,
                                 selector=fixed_strength_selector(), reward_reads=a.reads,
                                 repetitions=1, seed=MEASURE_BASE + 13 * tag)
        except Exception:
            return None
        o = out[0]
        return float(o.utility) if o.returned_valid and o.utility is not None else None

    rows, invalid, over_cap, short = [], 0, 0, 0
    started = time.time()
    for k, task in enumerate(tasks):
        base = mm(task.logical, task.host, 5000 + k)
        if base is None:
            continue
        base_q = sum(len(c) for c in base.values())
        base_u = measure(task, base, k * 1000)
        if base_u is None:
            continue
        per_alpha = {}
        for ai, (mode, alpha) in enumerate((m, al) for m in modes for al in alphas):
            best = None
            for rep in range(a.repeats):
                grown, missing = grown_embedding(task.host, base, alpha, a.lmin, a.lmax, rng,
                                                 mode=mode, logical=task.logical)
                q = sum(len(c) for c in grown.values())
                if q > a.qubit_cap:
                    over_cap += 1
                    continue
                if not realises(grown, task.logical, task.host):
                    invalid += 1
                    continue
                short += missing
                u = measure(task, grown, k * 1000 + 17 * ai + rep + 1)
                if u is None:
                    continue
                lengths = [len(c) for c in grown.values()]
                if best is None or u > best[1]:
                    best = (q, u, float(np.mean(lengths)), int(max(lengths)))
            if best is not None:
                per_alpha[(mode, alpha)] = best
        if per_alpha:
            rows.append({"base_q": base_q, "base_u": base_u, "per_alpha": per_alpha})

    print("  instances swept %d, invalid growths %d, over cap %d, qubits short of target %d, %.0fs"
          % (len(rows), invalid, over_cap, short, time.time() - started), flush=True)
    if not rows:
        print("  nothing to report"); return 1

    print("\n  %-8s %-9s %-11s %-11s %-9s %s"
          % ("alpha", "qubits", "mean chain", "max chain", "quality", "vs the minorminer start"))
    base_q = float(np.mean([r["base_q"] for r in rows]))
    base_u = float(np.mean([r["base_u"] for r in rows]))
    print("  %-11s %-7s %-9.1f %-11s %-9.4f %s" % ("start", "-", base_q, "-", base_u, "-"))
    for key in [(m, al) for m in modes for al in alphas]:
        have = [r for r in rows if key in r["per_alpha"]]
        if len(have) < 5:
            continue
        q = np.array([r["per_alpha"][key][0] for r in have], dtype=float)
        u = np.array([r["per_alpha"][key][1] for r in have], dtype=float)
        mc = np.array([r["per_alpha"][key][2] for r in have], dtype=float)
        d = u - np.array([r["base_u"] for r in have], dtype=float)
        rng2 = np.random.default_rng(0)
        bs = [d[rng2.integers(0, len(d), len(d))].mean() for _ in range(4000)]
        print("  %-11s %-7.1f %-9.1f %-11.2f %-9.4f %+.4f [%+.4f, %+.4f] over %d"
              % (key[0], key[1], q.mean(), mc.mean(), u.mean(), d.mean(),
                 np.percentile(bs, 2.5), np.percentile(bs, 97.5), len(have)))

    print("\n  Growth only adds qubits, so these are nested in resource: every row realises the")
    print("  same couplings as the row above it and costs more. A quality column that rises with")
    print("  the qubit column is the advisor's claim that unused hardware is an opportunity; one")
    print("  that falls is the opposite; one that does nothing says the budget is not the")
    print("  variable, and this time the comparison is a budget sweep rather than a perturbation.")
    print("\nBUDGET SWEEP DONE", flush=True)
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
