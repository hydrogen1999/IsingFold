...
twenty repaired states without spending a read.
    ...
    python3 probes/propose_rank.py --actor runs/ladder3_if-core/if-core_s2.pt --family if-core --k 8
"""
...
ap.add_argument("--ranker", default=None,
                help="checkpoint whose utility critic ranks the proposals; defaults to the actor, "
                     "which is what the specification implies and what fails")
...
model.load_state_dict(torch.load(a.actor, map_location="cpu"))
model.eval()
ranker = model
if a.ranker:
    ranker = build_model(a.family, improvement_mode=True)
    ranker.load_state_dict(torch.load(a.ranker, map_location="cpu"))
    ranker.eval()
...
def rollout(task, seed, rng):
    ...
    for _ in range(64):
        ...
        if ranker is model:
            ...
            with torch.no_grad():
                rv = ranker.forward_single(state.observation, None)
            last_value = float(rv.utility_value.detach().cpu().reshape(-1)[0])
...
if rows:
    ...
    print("%s, %d lineages, K=%d proposed per lineage, ranker=%s"
          % (a.family, len(rows), a.k, a.ranker or "the actor itself"))
