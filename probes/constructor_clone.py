"""Behaviour cloning of the witness's construction, then the same held-out evaluation.

The unhinted witness replay now reaches a valid COMMIT under the wide support, so the
environment can produce, for every train instance, a trajectory of states whose offered
candidates contain an action consistent with the planted witness. This trains the same
actor to put its mass on those actions: a set-valued loss, because several offered
candidates can be witness-consistent and picking one arbitrarily teaches an arbitrary
tie-break.

    L = -log sum_{a in S(s)} pi(a | s),  S(s) = witness-consistent legal candidates
...
def teacher_steps(task, witness, features, max_steps, seconds, wide=True, rng=None):
    """Walk the environment along the witness, recording at each step the candidate rows and
    which of them are witness-consistent. The walk itself is the replay's: take the first
    consistent candidate, COMMIT when offered."""
...
def main(argv=None):
    ...
    print(json.dumps({"protocol": "constructor-behaviour-cloning-v1", "corpus": a.corpus,
                      ...
                      "teacher": "set-valued over witness-consistent candidates; train lineages only",
