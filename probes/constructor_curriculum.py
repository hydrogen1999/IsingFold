...
# Legacy baseline probes import this variable. Bind them to the same checkout as
# this runner so a standalone invocation cannot fail or mix source revisions.
os.environ["ISINGFOLD_SRC"] = str(ROOT / "src")
...
FEATURE_WIDTHS = {"tiny": FEATURE_WIDTH, "local": FEATURE_WIDTH + LOCAL_CHANNELS,
                  "construction": CONSTRUCTION_WIDTH, "physics": PHYSICS_WIDTH}
...
def generate(stage, count, seed, lineage, exclude=(), max_attempts=2000):
    ...
    for attempt in range(max_attempts):
        ...
        # Each generated graph pair is a fresh parent instance, not an augmented
        # version of every other graph drawn under the same split/seed label.
        task.lineage = task.name
...
def manifest_split_sets(tasks, split, cells, n_train, n_heldout, seed, heldout_role="validation"):
    ...
    # Names can be instance IDs or lineage IDs. Check their resolved lineages, not
    # merely string overlap, before selecting any subset or cell.
    memberships = {}
    for task in tasks:
        lineage = task.lineage or task.name
        roles = memberships.setdefault(lineage, set())
        for role, names in (("train", train_names), ("validation", val_names), ("test", test_names)):
            if member(task, names):
                roles.add(role)
    if any(len(roles) > 1 for roles in memberships.values()):
        raise ValueError("manifest splits overlap at the lineage level")
    keep = [t for t in tasks if not cells or any(cell in t.name for cell in cells)]
    ...
    # One representative per lineage is the independent experimental unit.
    def unique(pool):
        by_lineage = {}
        for task in sorted(pool, key=lambda t: t.name):
            by_lineage.setdefault(task.lineage or task.name, task)
        return list(by_lineage.values())
    train_pool, val_pool = unique(train_pool), unique(val_pool)
    rng = np.random.default_rng(seed)
...
def load_init(path, actor, kind, features, expand=False):
    ...
    if expand and kind == blob.get("actor") == "linear" and (saved, features) in {
            ("tiny", "local"), ("tiny", "physics"), ("local", "physics")}:
        state = blob["state"]
        target = actor.state_dict()
        if list(state) != list(target) or len(state) != 1:
            raise ValueError("feature expansion requires the one-matrix linear actor")
        key = next(iter(state))
        padded = torch.zeros_like(target[key])
        old = state[key]
        if old.shape != (1, FEATURE_WIDTHS[saved]) or padded.shape != (1, FEATURE_WIDTHS[features]):
            raise ValueError("checkpoint feature width is inconsistent")
        padded[:, :old.shape[1]] = old
        actor.load_state_dict({key: padded})
        return {"expanded_from": saved, "zero_new_weights": True, "source_summary": blob.get("summary")}
...
def opcode_channel(features, name):
    ...
    if features in ("tiny", "local", "physics"):
...
def make_features(kind, task):
    ...
    if kind == "physics":
        return PhysicsFeatures(task)
    if kind == "construction":
        return ConstructorFeatureContext(task, len(task.host))
    raise ValueError("features must be tiny, local, physics or construction")
...
def run(args, train, heldout):
    ...
    def evaluate_quality(tag):
        ...
        if args.heldout_role == "test":
            sets = (("heldout", heldout),)
        for name, tasks in sets:
            ...
            for k, t in enumerate(tasks):
                ...
                if args.comparison == "all":
                    # growth of zero to four qubits (including the unchanged draw), the count drawn per
                ...
                if experiment_seed(args.seed, "arm-order", t.name) % 2:
                    arms.reverse()
                row = {"instance": t.name, "lineage": t.lineage or t.name}
                for arm, proposer in arms:
                    from constructor_objective import measure_terminal_metrics, measure_selection_energy, quality_utility
                    search = evaluate_search
                    extra = {}
                    if args.proposal_fraction:
                        from constructor_protocol import evaluate_anytime_search
                        search = evaluate_anytime_search
                        extra["proposal_fraction"] = args.proposal_fraction
                    row[arm] = search(t, proposer, deadline=args.deadline, select_cap=args.select_cap,
                                               ...
                                               measure=measure_terminal_metrics, select_measure=measure_selection_energy,
                                               utility=quality_utility, **extra)
                    if row[arm]["valid"] and not row[arm]["assessment_ok"]:
                        raise ValueError("a valid chosen embedding has no independent assessment")
                rows.append(row)
            valid = {arm: float(np.mean([r[arm]["valid"] for r in rows]))
                     for arm in rows[0] if arm not in ("instance", "lineage")}
            ...
            paired_all = {arm: paired_against(arm) for arm in rows[0] if arm not in ("instance", "lineage", "policy")}
            ...
            summary = {"evaluation": tag, "set": name, "seed": args.seed, "stage": args.stage,
                       ...
                       "deployment_utility": {arm: float(np.mean([r[arm]["quality_utility"] for r in rows]))
                                              for arm in valid},
                       "deployment_p_solve": {arm: float(np.mean([r[arm]["p_solve"] or 0.0 for r in rows]))
                                              for arm in valid},
                       "assessment_coverage": {arm: float(np.mean([r[arm]["assessment_ok"] for r in rows]))
                                               for arm in valid},
    ...
    contract = run_contract(args, train, heldout)
    ...
    print(json.dumps({
        ...
        "objective": args.objective, "evaluation_objective": evaluation_objective,
        ...
        "entropy_coef": args.entropy_coef, "advantage_scale": args.advantage_scale,
        "prefix_curriculum": args.prefix_fraction or None,
    ...
    first_tag = "final" if args.iterations == 0 else "init"
    first_eval = evaluate(first_tag)
    history = {"init": first_eval}
    if args.iterations:
        checkpoints.consider(actor, first_eval, "init")
    ...
    for iteration in range(args.iterations):
        ...
        for slot, idx in enumerate(picks):
            ...
            loss, metrics = constructor_loss(records, baseline=args.baseline,
        ...
        if (iteration + 1) % args.eval_every == 0 and iteration + 1 < args.iterations:
            ...
            history[tag] = evaluate(tag)
            checkpoints.consider(actor, history[tag], tag)
    history["final"] = evaluate("final") if args.iterations else first_eval
    if args.iterations:
        checkpoints.consider(actor, history["final"], "final")
    ...
    summary["selected_checkpoint"] = {"tag": checkpoints.best_tag,
                                       "validation_score": checkpoints.best_score if checkpoints.best_tag is not None else None}
    print(json.dumps(summary), flush=True)
    if args.out:
...
def parse(argv=None):
    ...
    parser.add_argument("--expand-features", action="store_true",
                        help="linear only: zero-pad tiny -> local/physics or local -> physics; preserve initial logits")
    ...
    parser.add_argument("--evaluation-objective", choices=("feasibility", "quality"), default=None,
                        help="optional evaluation endpoint independent of training reward; use quality "
                             "for both quality training and the continued-feasibility control")
    ...
    parser.add_argument("--proposal-fraction", type=float, default=0.,
                        help="0: equal-K sequential measurement; (0,1): spend this fraction of the total "
                             "deadline forming a structural-diversity shortlist before measurement, for every arm")
    ...
    parser.add_argument("--comparison", choices=("none", "minorminer", "minorminer_grown", "all"),
                        ...
                        help="all: minorminer, plus fixed random growth, plus growth of 0 to 4 chosen by "
    ...
    parser.add_argument("--data-seed", type=int, default=None,
                        help="fixed instance/split seed independent of the training seed (default: --seed)")
    ...
    if not np.isfinite(args.entropy_coef) or args.entropy_coef < 0:
        parser.error("entropy coefficient must be nonnegative and finite")
    if not 0 <= args.proposal_fraction < 1:
        parser.error("proposal fraction must be in [0,1)")
    if args.expand_features and (not args.init or args.actor != "linear"):
        parser.error("feature expansion requires --init and a linear actor")
    ...
    if args.manifest_split and args.stage != "corpus":
        parser.error("--manifest-split requires --stage corpus")
    if args.objective == "deployment" and args.iterations:
        parser.error("--objective deployment is evaluation only: use --iterations 0")
    if args.objective == "deployment" and args.evaluation_objective:
        parser.error("deployment mode cannot override its evaluation objective")
...
def grown_baseline_proposal(task, budget, seed, seconds_left, extra, tries=2, router_seconds=2.0,
    ...
    if chains is None:
        return None
    rng = np.random.default_rng(seed)
    count = int(rng.integers(0, 5)) if extra is None else extra
    if rule == "heuristic":
...
def main(argv=None):
    ...
    if args.stage == "corpus":
        ...
        train, heldout = build_corpus_sets(args.corpus, cells, args.train, args.heldout, data_seed,
                                           use_manifest_split=args.manifest_split,
                                           heldout_role=args.heldout_role)
    else:
        train, heldout = build_sets(args.stage, args.train, args.heldout, data_seed)
