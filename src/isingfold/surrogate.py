...
from isingfold.embedding import Embedding, LogicalProblem, Node, Qubit
from isingfold.programming import program
...
def solve_probability_at(
    ...
    """Fraction of anneals that decode to the logical ground state, at one F."""
    ...
    for row in ss.record.sample:
        ...
        broken_total += broken
        if logical_energy(problem, spins) <= ground_energy + tol:
            hits += 1
...
def score_embedding(
    ...
    results = [
        solve_probability_at(
            ...
            num_reads=num_reads, num_sweeps=num_sweeps, seed=seed + k,
        )
