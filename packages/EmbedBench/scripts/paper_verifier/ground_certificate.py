...
from paper_verifier.schema import canonical_sha256, validate_versioned_object
...
def _require_ground_status(value: object) -> GroundStatus:
    if type(value) is not str or value not in _GROUND_STATUSES:
        raise ValueError(f"status must be one of {sorted(_GROUND_STATUSES)}")
    return cast(GroundStatus, value)
...
class IsingProblem:
    ...
    def from_logical_problem(cls, value: object) -> IsingProblem:
        ...
        from embedbench.embedding import LogicalProblem  # type: ignore[import-untyped]
...
class GroundStateCertificate:
    ...
    def __post_init__(self) -> None:
        ...
        _require_ground_status(self.status)
        ...
        if self.status == "uncertified_reference":
            ...
            if self.checker is None or self.checker.exit_code != 0:
    ...
    def from_dict(cls, value: object) -> GroundStateCertificate:
        ...
        status = _require_ground_status(document["status"])
        energy = Dyadic(
...
def build_exact_enumeration_certificate(
    ...
    *,
    ...
    """Enumerate every state in Gray-code order and persist its replayable header."""
    ...
    if not isinstance(checker, CheckerRun) or checker.exit_code != 0:
        raise ValueError("exact enumeration requires a successful checker run")
