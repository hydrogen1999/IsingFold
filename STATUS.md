...
| constructor ladder, corpus-size rung (branch `feat/constructor-curriculum`, `a64092f`) | stages F and G: 12 to 20 variables on 64 to 128 qubit fragments of Pegasus 3 and Zephyr 2, the size of the small corpora where the old constructor read 0.00; horizon 240, 300 s episodes, 100 iterations, 6 episodes an instance, 10 evaluation episodes; 16-channel linear three seeds, 230-channel linear one seed, contextual at lr 3e-3 one seed, per host | apollo `~/prj_IsingFold_pr2/runs/curriculum/{F,G}_linear_s*.log`, `{F,G}_lin230_s0.log`, `{F,G}_ctx_lr3e3_s0.log` | iteration 50 to 75 of 100: 16-channel linear held-out 0.80 / 0.81 / 0.75 (Pegasus 3 fragments) and 0.87 / 0.78 / 0.63 (Zephyr 2 fragments); 230-channel linear 0.93 (Zephyr, it 49); contextual at lr 3e-3 0.83 (Pegasus) and 0.92 (Zephyr); from 0.03 to 0.17. The size where the old constructor read 0.00 through 40 iterations. Seeds 1 and 2 of the 16-channel runs were stopped at iteration 60 to 100 to give the fill rung the cores; seed 0 and the 230-channel and contextual runs continue to 100 |
...
[-0.012, +0.004]. The stage-b trace does not reappear: at this rung the learned part's
quality contribution beyond spend-and-measure is zero on Zephyr; Pegasus running.
