...
| constructor ladder, size rung (branch `feat/constructor-curriculum`, `d765812`) | stages P and Z: 8 to 14 variables on 32 to 64 qubit fragments of Pegasus 3 and Zephyr 2, horizon 160 steps, 120 s episodes; 16-weight linear actor three seeds each, contextual actor-critic one seed each | apollo `~/prj_IsingFold_pr2/runs/curriculum/{P,Z}_linear_s*.log`, `{P,Z}_ctx_value_s0.log` | running; at iteration 50 to 150 the 230-channel linear actor is at held-out 0.94 (Pegasus 3 fragments, seed 1) and 0.92 / 0.83 (Zephyr 2 fragments) from 0.05 to 0.10, the 16-channel actor at 0.57 to 0.84; two more 230-channel seeds per host on goose |
