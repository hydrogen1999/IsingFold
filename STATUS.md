...
| hardware-sized rung (branch `feat/constructor-curriculum`, `7d1c989`) | 24-variable ink-drop instances on the full Pegasus 16 (5,640 qubits) and Zephyr 15 (7,440 qubits), train from the corpus' locked train list (16 tasks, witness prefix curriculum 90 percent down to none), held-out from its validation list (4 tasks, no witness), the test list untouched; 16-channel actor, horizon 400, 300 s episodes, 100 iterations | goose `~/prj_IsingFold_pr2/runs/curriculum/ink24_{pegasus16,zephyr15}_prefix_s0.log` | relaunched 2026-09-17 warm-started from the corpus-size checkpoints (`ink24_*_prefixinit_s0.log`), through Slurm as a two-core allocation (`scripts/goose/submit_curriculum_batch.sh`; the account allows two jobs in the queue, the partition had 247 idle CPUs): goose caps each ssh login session at one CPU (`cpu.max 100000 100000` on the session scope), so the first launch shared one core between two jobs and spent three hours in its initial evaluation. **First held-out validity at hardware size, from empty** (`ink24_*_prefixinit_s0.log`, iteration 24 of 100, 4 validation-list tasks, 5 episodes each): Pegasus 16 (5,640 qubits) 0.10 at the corpus-size checkpoint to 0.25; Zephyr 15 (7,440 qubits) 0.35 to 0.60; training validity 13/24 and 19/24 at prefix 0.64 to 0.73. Small problems on the paper's hardware; evaluation from empty, minorminer forbidden. Runs continue |
...
size where the previous constructor read 0.00 through 40 iterations):
    ...
    host, seed          train before -> after    held-out before -> after    held-out gain
    Pegasus 3, 0        0.04 -> 0.83             0.08 -> 0.80                +0.72 [+0.50, +0.90]
    Zephyr 2, 0         0.06 -> 0.74             0.17 -> 0.88                +0.71 [+0.53, +0.85]

Every interval above zero; the 230-channel linear actor sits higher again: Zephyr 2
fragments, seed 1, held-out 0.06 -> 0.96 (+0.90 [+0.81, +0.97], `Z_lin230_s1.log`) and
...
(`goose:~/prj_IsingFold_pr2/runs/curriculum/ink24_{pegasus16,zephyr15}_prefixinit_s0.log`):
    ...
    host          held-out at the checkpoint    held-out at iteration 24    training validity at 24 to 29
    Pegasus 16    0.10                          0.25                        13 / 24 (prefix 0.64)
    Zephyr 15     0.35                          0.60                        19 / 24 (prefix 0.73)
...
independent), and 25 iterations of the prefix curriculum double it on both. This is the
first learned validity on the hardware the paper names, at the small end of the size axis
(24 variables); the fill axis (263 to 533 variables at 80 to 95 percent) is the one that
decides the paper and is running on the wide support.
