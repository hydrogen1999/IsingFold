...
| hardware-sized rung (branch `feat/constructor-curriculum`, `7d1c989`) | 24-variable ink-drop instances on the full Pegasus 16 (5,640 qubits) and Zephyr 15 (7,440 qubits), train from the corpus' locked train list (16 tasks, witness prefix curriculum 90 percent down to none), held-out from its validation list (4 tasks, no witness), the test list untouched; 16-channel actor, horizon 400, 300 s episodes, 100 iterations | goose `~/prj_IsingFold_pr2/runs/curriculum/ink24_{pegasus16,zephyr15}_prefix_s0.log` | relaunched 2026-09-17 warm-started from the corpus-size checkpoints (`ink24_*_prefixinit_s0.log`), through Slurm as a two-core allocation (`scripts/goose/submit_curriculum_batch.sh`; the account allows two jobs in the queue, the partition had 247 idle CPUs): goose caps each ssh login session at one CPU (`cpu.max 100000 100000` on the session scope), so the first launch shared one core between two jobs and spent three hours in its initial evaluation. The account allows only a couple of jobs in the queue (`AssocMaxSubmitJobLimit`), so the twelve warm-started prefix runs at fill 85 and 95 (seed 0) and seeds 1 and 2 at fill 80 and 90, both hosts, go as one twelve-core job (`scripts/goose/submit_curriculum_batch.sh`, `runs/curriculum/fill*_prefixinit_s*.log` on goose) |
...
(-0.004 to -0.018), present but not a headline. The quality story stays with the platform.
