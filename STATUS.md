...
`z_*.log`):
    ...
    Pegasus 2, 0        0.77 (it 180)            0.75 (it 180)               running
...
The rule the 16-weight actor learns transfers to the target topologies at this scale. Gate 3
(the PR's 230-channel contextual actor-critic on the stage-b sets) at iteration 50 of 200:
held-out 0.87 and 0.80 with the value baseline, 0.73 to 0.74 with leave-one-out, against
0.53 to 0.62 for the 16-weight actor at the same iteration; the linear actor on the 230
channels trains faster too (0.77 to 0.85 train at iteration 50 against 0.43 to 0.50). Final
numbers pending. Next rungs in order: wall-clock and size (12 to 20 variables on Pegasus 3
and Zephyr 2 fragments, then the full small hosts), then quality.
