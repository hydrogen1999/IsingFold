...
project with a positive held-out number; it is small (at most 4 variables, hosts of 6 to
15 qubits) and it is feasibility, not annealing quality. Stage b (4 to 8 variables on grids
with holes and dead ends) is at iteration 50 of 200 with held-out 0.53 to 0.62 from 0.09 to
0.15 and still climbing; a small MLP actor is sharper (entropy 0.43 against 0.9) and is
being compared on the same sets. Next rungs in order: finish stage b with three seeds, then
Pegasus and Zephyr fragments at the same variable counts, then the 230-channel contextual
actor on the same sets (gate 3), then wall-clock and size, then quality.
