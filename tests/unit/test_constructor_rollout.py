...
def test_missing_quality_is_not_a_fabricated_success_reward():
    result, _, _ = run(grown_plan(), objective="quality", train=True, measure=lambda *args: None)
    assert result["valid"] and result["unmeasured_valid"]
    assert result["quality_status"] == "missing" and result["quality_coverage"] == 0
    assert result["return"] == result["base_return"] == 0
